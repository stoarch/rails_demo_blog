...
author_templates = [
  {full_name: 'John Doe', birth_date: Date.new(1980,1,1) },
  {full_name: 'Nick Warren', birth_date: Date.new(1981,2,1) },
  {full_name: 'Loren Straight', birth_date: Date.new(1982,3,1) },
  {full_name: 'Paul Smith', birth_date: Date.new(1988,5,1) }
]

authors = []

author_templates.each do |at|
  authors << Author.where(full_name: at[:full_name]).first_or_create(at)
end
...
authors.each_with_index do |a,i|
  2.times do |j|
    ...
    a.posts.where(title: post_title).first_or_create({title: post_title, content: lorem_ipsum })
  end
end
